...
class MineIOonLiveData<T>(private val get: (T) -> T, private val nullFlag: T){
    ...
    constructor(get: (T) -> T, set: (T) -> T, nullFlag: T): this(get,nullFlag){
        tempSet = set
        setter = {
            Log.d("MineIOonLiveData","set begin ${setteblyValue.toString()} ${pValue.value.toString()} => $tempSet")
            val t = tempSet(setteblyValue)
            //if (pValue.value != t)
                pValue.value = t
            Log.d("MineIOonLiveData","set end ${setteblyValue.toString()} ${pValue.value.toString()} <= $tempSet")
        }
    }
    ...
    var value: T
        get() {
            Log.d("MineIOonLiveData","get begin ${pValue.value.toString()} => $get")
            val t = get(pValue.value ?: nullFlag)
            if (pValue.value != t) pValue.value = t
            Log.d("MineIOonLiveData","get end ${t.toString()} <= $get")
            return t
        }
        set(value) {
            setteblyValue = value
